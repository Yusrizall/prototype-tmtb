...
The Spear must be defeated before the Structure lesson begins in the current authored Tutorial. The older v3.2 free-priority ordering after first Structure interaction is **SUPERSEDED**.
...
The first meaningful Tutorial Objective is a destroy-Structure lesson. It begins only after the Spear lesson has been completed and the Spear is defeated. The first Structure interaction is guided; afterward the Player finishes the Structure through normal combat rules.

This sequencing is an authored Tutorial curriculum decision, not a universal rule that combat Objectives must always wait for enemy elimination.
...
- `CHARGE` as an enemy Intent that builds over multiple Enemy activations;
- `CHARGE X/Y` as progress toward completion;
- a Charge activation advances preparation rather than resolving the payoff immediately;
- when Charge completes, the prepared payoff becomes the enemy's next/current readable Intent;
...
→ step-by-step tactical guidance OFF
→ optional high-level graduation framing remains
→ normal-combat free play
→ additional required Wave pressure may continue if needed by the current validation PVS
```
...
Final free play keeps normal combat information such as Team AP, Objective, roster, control hints, Enemy Intent, Status/Charge, range/Cover feedback, and Wave Telegraph. A concise non-tactical graduation prompt may remain; exact wording is supporting/prototype presentation detail.
...
Exact Wave count, composition, timing, spawn geometry, fallback positions, and current balancing response to Archer AP funneling remain supporting/PVS detail.
...
The validation update is newer for conflicting post-baseline Phase 6–8 detail.
